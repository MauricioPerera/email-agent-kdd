...
- Install from the plugin with `powershell -File <PLUGIN_ROOT>\scripts\install-cli.ps1` (Windows) or `sh <PLUGIN_ROOT>/scripts/install-cli.sh` (macOS/Linux). Both download the stable `v0.2.2` CLI wheel and bundled dependencies, verify every SHA-256 before invoking pip, and verify `email-agent --help` when finished.
