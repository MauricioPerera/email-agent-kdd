...
- Install with the platform-appropriate installer: `powershell -File installers\install.ps1` (Windows) or `sh installers/install.sh` (macOS and Linux); both use the stable `v0.1.0` release by default and verify the command when they finish. Alternatively `python -m pip install .`.
