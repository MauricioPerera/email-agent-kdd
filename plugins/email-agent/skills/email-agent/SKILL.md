...
- Extract text from an already stored, safe text attachment: `attachment extract ROOT REL_PATH INDEX DEST CONFIRMAR EXTRACCION`. This is local-only, requires the exact user confirmation, supports only inert UTF-8 `text/plain`, `text/csv`, `text/markdown`, and `application/json`, validates the blob hash and size before writing, and never sends or changes the remote message. HTML, PDF, office documents and executable formats remain unsupported until a sandboxed parser and antivirus integration exist.
