...
- Install from the plugin with `powershell -File <PLUGIN_ROOT>\scripts\install-cli.ps1` (Windows) or `sh <PLUGIN_ROOT>/scripts/install-cli.sh` (macOS/Linux). Both download the stable `v0.2.1` CLI wheel and bundled dependencies, verify every SHA-256 before invoking pip, and verify `email-agent --help` when finished.
...
- Address filters use boundaries, so a longer address that merely contains the requested text does not match.
...
Remote identity: use the stored `uidvalidity` together with account, mailbox and
UID. Remote delete/restore/purge require `--uidvalidity N` from that original
reference; never invent N or substitute the server's current generation to make
...
- Extract text from an already stored attachment: `attachment extract ROOT REL_PATH INDEX DEST CONFIRMAR EXTRACCION`. This is local-only, requires the exact user confirmation and a clean antivirus result, validates the blob hash and size before writing, and never sends or changes the remote message. Inert UTF-8 text/plain, text/csv, text/markdown and application/json are supported; PDF has the additional sandbox requirements below. HTML, office documents and executable formats remain unsupported.
- PDF is supported separately from inert text only on Linux with Bubblewrap, prlimit and /usr/bin/python3. It uses the same antivirus gate, isolated network, restricted files, clean environment, 256 MiB memory, 25 MiB input, 100 pages, 4 MiB text and 15 seconds. Windows/macOS and unavailable sandbox runtimes return pdf-sandbox-unavailable; do not launch the parser directly as a workaround. Encrypted, corrupt or limit-exceeding PDFs are rejected; HTML and office documents remain blocked.
