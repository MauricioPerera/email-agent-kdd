...
- Extract text from an already stored, safe text attachment: `attachment extract ROOT REL_PATH INDEX DEST CONFIRMAR EXTRACCION`. This is local-only, requires the exact user confirmation, runs the configured local antivirus gate fail-closed, supports only inert UTF-8 `text/plain`, `text/csv`, `text/markdown`, and `application/json`, validates the blob hash and size before writing, and never sends or changes the remote message. HTML, PDF, office documents and executable formats remain unsupported until a sandboxed parser is available.
- PDF text extraction uses the same antivirus gate and a separate no-network worker with limits of 25 MiB input, 100 pages, 4 MiB parser output and 15 seconds. Encrypted, corrupt or limit-exceeding PDFs are rejected; HTML and office documents remain blocked.
