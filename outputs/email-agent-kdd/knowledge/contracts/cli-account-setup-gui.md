...
intent: agregar el subcomando account setup-gui ROOT con un formulario local para usuarios no tecnicos que guarda el secreto solo en el almacenamiento seguro de Windows
...
5. Llamar UNA sola vez `provision_windows_email_account(root, account_id, "custom", email, label, password)` pasando el secreto directo desde el widget, solo en memoria (sin copiarlo a atributos, logs, archivos ni variables de entorno).
   - `RuntimeError` de provision (PARADA de wincred: sistema no Windows o Credential Manager indisponible): PARADA clara que indique que el almacenamiento seguro de Windows no esta disponible y que NO existe alternativa segura; JAMAS ofrecer fallback (ni variable de entorno, ni archivo, ni texto en claro); limpiar password, cerrar, retornar `1`. En este caso NO se llama a `store_mail_server_config`.
...
- La referencia `wincred://<label>` va UNICAMENTE en `accounts.json` (via `save_email_account` dentro de provision); los hosts y puertos van UNICAMENTE en `.email-agent/mail-servers.json` (via `store_mail_server_config`); JAMAS un host en `accounts.json` ni el secreto en `mail-servers.json`.
- Orden congelado al guardar: `discover_mail_servers(email)` (o la seccion avanzada) ANTES de `provision_windows_email_account`, y provision ANTES de `store_mail_server_config`. Si el discovery fallo sin datos avanzados completos, NO hay provision ni escritura alguna. Si falla la provision NO se llama al almacen de servidores.
...
- En no-Windows o sin Credential Manager: PARAR claro en la ventana y JAMAS un fallback inseguro (ni env, ni archivo plano, ni texto en claro, ni keyring).
...
- Sin procesos, sin `eval`/`exec`; las unicas escrituras en disco son las que hacen `provision_windows_email_account` (via `save_email_account`) y `store_mail_server_config` dentro del root indicado; el formulario no escribe nada por su cuenta.
...
- Ejemplo frozen: con los valores del bloque `frozen-inputs`, pulsar Guardar produce EXACTAMENTE UNA llamada `discover_mail_servers("Ana@Example.COM")` (sin contrasena, sin resolver), que devuelve el dict descubierto; UNA confirmacion publica en la ventana; UNA llamada `provision_windows_email_account("<temp-oracle>", "ana-example.com", "custom", "Ana@Example.COM", "email-ana-example.com", "app-pass-fake-123")` (email SOLO recortado y sin cambiar mayusculas/minusculas: la normalizacion ocurre DENTRO de provision via `create_email_account`; password verbatim); UNA llamada `store_mail_server_config("<temp-oracle>", "ana-example.com", {"imap_host": "imap.gmail.com", "imap_port": 993, "smtp_host": "smtp.gmail.com", "smtp_port": 587})`; una confirmacion publica `cuenta guardada: ana-example.com`; el campo password limpio; codigo `0`; en `accounts.json` la ref `wincred://email-ana-example.com` JAMAS el secreto; en `mail-servers.json` solo los hosts.
...
- Provision lanzando `RuntimeError` de PARADA (no-Windows o Credential Manager indisponible): PARADA clara en la ventana sin ofrecer alternativa alguna, password limpio, codigo `1`, y `store_mail_server_config` NO se llama.
...
- Don't: ofrecer fallback inseguro en no-Windows o sin Credential Manager; PARAR con un mensaje claro y sin alternativas.
